                    ...
                    >
                        <template v-slot:body="props">
                            <q-tr :props="props">
                                ...
                                <q-td key="ops" v-else-if="mode === 'supervisor'" :props="props">
                                  <a class="text-blue" style="cursor: pointer; padding: 5px;" @click="$emit('addsupervisor', props.row)"> <q-icon size="md" name="add"/>
                                    <q-tooltip :delay="1000" :offset="[0, 10]">agregar</q-tooltip>
                                  </a>
                                </q-td>
    ...
    async getUsers () {
      try {
        ...
        if (this.mode === 'project') us = await UserService.getClients({ token: localStorage.getItem('token') })
          ...
          else if (this.mode === 'supervisor') us = await UserService.getSupervisors({ token: localStorage.getItem('token') })
